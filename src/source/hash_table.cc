...
Value HashTable<Key, Value>::Get(Key key) {
  ...
  Value value = {};
  for (auto itr = buckets_[hash_key].begin();
       itr != buckets_[hash_key].end() && !is_find; ++itr) {
    if (key == itr->first) {
      is_find = true;
      value = itr->second;
    }
...
bool HashTable<Key, Value>::Exists(Key key) {
  ...
  int hash_key = Hash(key);
  for (auto itr = buckets_[hash_key].begin();
       itr != buckets_[hash_key].end() && !is_exists; ++itr) {
    if (key == itr->first) {
      is_exists = true;
    }
  }
...
bool HashTable<Key, Value>::Del(Key key) {
  bool result = false;
  int hash_key = Hash(key);
  auto delete_node = buckets_[hash_key].begin();
  for (auto itr = buckets_[hash_key].begin();
       itr != buckets_[hash_key].end() && !result; ++itr) {
    if (key == (*itr).first) {
      delete_node = itr;
      result = true;
    }
  }
  buckets_[hash_key].erase(delete_node);
  return result;
}
